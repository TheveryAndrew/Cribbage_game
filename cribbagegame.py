...
class CribbageGame():
    ...
    def godeclaration(self):
        ...
        cardscomputervalid=self.validcards(self.hand)
                            ...
                            if (selectedcard := self.clickcard(posx, posy, 6))!=None:
                                ...
                                if len(self.selectedcards)==2:
                                    ...
                                    if self.whosturn==YOU:
                                        ...
                                        self.currentdrawing='drawcomputercards'
                                        self.theturncard=random.sample(self.cardlist, 1)[0]
                                        self.cardlist.remove(self.theturncard)
    ...
    def personpegging(self, posx : float, posy : float, listofpeggingcards : list):
        ...
        if len(cardsthatarevalid)>0:
            if peggingcard in cardsthatarevalid:
                ...
                somepoints=self.peggingpoints(self.pegginglist)
    ...
    def computerpegging(self, listofpeggingcards):
        ...
        computercardsthatarevalid = [card for card in self.hand if card[-1] + total <=31]
        if len(computercardsthatarevalid)>0:
            ...
            del self.hand[self.hand.index(computerspick)]
            self.countingpileforcomputer.append(computerspick)
            self.whosturn=YOU
        else:
    ...
    def removedumaque(self, is_there_a_dumaque : bool):
        if is_there_a_dumaque==False:
            ...
            self.rollingballs.remove('dumaque')
    def peggingpoints(self, listofpeggingcards):
